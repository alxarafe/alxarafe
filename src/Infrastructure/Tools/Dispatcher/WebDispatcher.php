...
use Modules\Admin\Controller\ErrorController;
...
class WebDispatcher extends Dispatcher
{
    ...
    public static function run(string $module, string $controller, string $method): bool
    {
        ...
        if ($endpoint === null) {
            static::dieWithMessage(\Alxarafe\Infrastructure\Lib\Trans::_('dispatcher_module_controller_not_found', ['module' => $module, 'controller' => $controller]));
        }
        ...
        if (!class_exists($className)) {
            if (!file_exists($filename)) {
                static::dieWithMessage(\Alxarafe\Infrastructure\Lib\Trans::_('dispatcher_file_not_found', ['file' => $filename]));
            }
        ...
        $theme = 'default';
        $language = \Alxarafe\Infrastructure\Lib\Trans::FALLBACK_LANG;
        ...
        \Alxarafe\Infrastructure\Lib\Trans::setLang($language);

        $controllerClass = new $className();
        if (!$controllerClass instanceof ViewController) {
            static::dieWithMessage(\Alxarafe\Infrastructure\Lib\Trans::_('dispatcher_not_view_controller', ['class' => $className]));
        }
        ...
        if (!method_exists($controllerClass, $method)) {
            Debug::message(\Alxarafe\Infrastructure\Lib\Trans::_('dispatcher_method_not_found', ['method' => $method, 'class' => $className]));
            $method = 'index';
        }

...
abstract class GenericController
{
    ...
    public function __construct(
        ...
        try {
            ...
            $newSidebarItems = \Modules\Admin\Service\MenuManager::get('admin_sidebar');

            \Alxarafe\Infrastructure\Tools\Debug::message("MenuManager Items: " . json_encode($newSidebarItems));
            ...
            \Alxarafe\Infrastructure\Tools\Debug::message("MenuManager Error: " . $e->getMessage());
        }

        // New Menu System Integration
        try {
            $this->data['header_user_menu'] = \Modules\Admin\Service\MenuManager::get('header_user');
        } catch (\Throwable $e) {
    ...
    protected function executeAction(): bool
    {
        ...
        if (!$isPublic && \Alxarafe\Infrastructure\Auth\Auth::$user) {
            if (!\Alxarafe\Infrastructure\Auth\Auth::$user->can($this->action, static::getControllerName(), static::getModuleName())) {
                ...
                Debug::message($msg);
                \Alxarafe\Infrastructure\Lib\Functions::httpRedirect("index.php?module=Admin&controller=Error&action=index&message=" . urlencode($msg));
                return false;
            }
        }
    ...
    public static function getModuleName(): string
    {
        ...
        if (count($parts) >= 2 && ($parts[0] === 'Modules' || $parts[0] === 'Modules')) {
            return $parts[1];
        }
        return '';
    }

...
trait ResourceTrait
{
    ...
    /**
     * Prefix for extrafields form field names to avoid collisions.
     */
    ...
    /**
     * @var \Alxarafe\Infrastructure\Persistence\Model\Model|null Cached record instance.
     */
    protected ?\Alxarafe\Infrastructure\Persistence\Model\Model $record = null;
    ...
    protected function setup()
    {
        ...
        $this->addListButton(
            'new',
            \Alxarafe\Infrastructure\Lib\Trans::_('new'),
            'fas fa-plus',
            'primary',
            'right',
            'url',
        ...
        $this->addEditButton(
            'save',
            \Alxarafe\Infrastructure\Lib\Trans::_('save_changes'),
            'fas fa-save',
            'primary',
            'right',
            'submit'
        ...
        $this->addEditButton(
            'back',
            \Alxarafe\Infrastructure\Lib\Trans::_('back'),
            'fas fa-arrow-left',
            'secondary',
            'right',
            'url',
     ...
     * 
     * @param \Alxarafe\Infrastructure\Persistence\Model\Model $model The saved model instance.
     * @param array $data The original submitted data.
     */
    protected function afterSaveRecord(\Alxarafe\Infrastructure\Persistence\Model\Model $model, array $data)
    {
}
        ...
        if (!empty($tabsConfig)) {
            ...
            foreach ($tabsConfig as $sectionId => $sectionData) {
                ...
                $sectionFields = array_values(array_filter($sectionFields, function (mixed $fieldObj): bool {
                    if ($fieldObj instanceof \Alxarafe\Infrastructure\Component\AbstractField) {
                        return $fieldObj->isVisible();
                    } elseif ($fieldObj instanceof \Alxarafe\Infrastructure\Component\Container\AbstractContainer) { // @phpstan-ignore instanceof.alwaysFalse
                        // Containers (Panel, etc.) — check recursively
                        $fieldObj->filterChildren(fn($child) =>
                            !($child instanceof \Alxarafe\Infrastructure\Component\AbstractField) || $child->isVisible());
                    }
                    return true;
                }));
                ...
                if (!empty($fieldsByName)) {
                    foreach ($sectionFields as $fieldObj) {
                        if (!($fieldObj instanceof \Alxarafe\Infrastructure\Component\AbstractField)) continue;
    ...
    protected function convertModelFieldsToComponents(array $modelFields): array
    {
        ...
        foreach ($modelFields as $fieldData) {
            ...
            switch ($type) {
                case 'boolean':
                    $components[] = new \Alxarafe\Infrastructure\Component\Fields\Boolean($field, $label, $options);
                    break;
                case 'date':
                    $components[] = new \Alxarafe\Infrastructure\Component\Fields\Date($field, $label, $options);
                    break;
                case 'datetime':
                    $components[] = new \Alxarafe\Infrastructure\Component\Fields\DateTime($field, $label, $options);
                    break;
                case 'time':
                    $components[] = new \Alxarafe\Infrastructure\Component\Fields\Time($field, $label, $options);
                    break;
                case 'integer':
                    // Integer Logic
                    ...
                    $components[] = new \Alxarafe\Infrastructure\Component\Fields\Integer($field, $label, $options);
                    break;
                case 'decimal':
                    // Decimal Logic
                    if (str_contains($dbType, 'unsigned')) {
                    ...
                    $components[] = new \Alxarafe\Infrastructure\Component\Fields\Decimal($field, $label, $options);
                    break;
                case 'textarea':
                    $options['multiline'] = true;
                    $components[] = new \Alxarafe\Infrastructure\Component\Fields\Textarea($field, $label, $options);
                    break;
                ...
                default:
                    $components[] = new \Alxarafe\Infrastructure\Component\Fields\Text($field, $label, $options);
                    break;
            }
        }
    ...
    protected function fetchListData(string $tabId): array
    {
        ...
        /** @var \Alxarafe\Infrastructure\Persistence\Model\Model $model */
    ...
    protected function processResultModels($models, $columns, $total, $limit): array
    {
        ...
        foreach ($models as $item) {
            ...
            // \Alxarafe\Infrastructure\Tools\Debug::message("Row " . $item->id . " keys: " . implode(',', array_keys($row)));
     ...
     *
     * @return \Alxarafe\Infrastructure\Persistence\Model\Model|null
     */
    protected function getRecord(): ?\Alxarafe\Infrastructure\Persistence\Model\Model
    {
        if ($this->record === null && $this->recordId && $this->recordId !== 'new') {
            ...
            if ($modelClass && class_exists($modelClass)) {
                /** @var class-string<\Alxarafe\Infrastructure\Persistence\Model\Model> $modelClass */
                /** @var \Alxarafe\Infrastructure\Persistence\Model\Model|null $record */
    ...
    protected function saveRecord()
    {
        ...
        $collectFields = function ($fields) use (&$fieldDefs, &$collectFields) {
            foreach ($fields as $f) {
                if ($f instanceof \Alxarafe\Infrastructure\Component\Container\Panel) {
                    $collectFields($f->getFields());
                } elseif ($f instanceof \Alxarafe\Infrastructure\Component\AbstractField) {
    ...
    protected function renderView()
    {
        ...
        if (empty($this->backUrl) && $this->mode === \Alxarafe\Infrastructure\Http\Controller\Interface\ResourceInterface::MODE_EDIT) {
        ...
        // Workflow: pass available transitions and current state to the view
        $workflowTransitions = [];
        ...
        if (
            ...
            && $this->recordId
            && $this->recordId !== 'new'
            && method_exists($this->modelClass, 'getAvailableTransitions')
        ) {
            $wfModel = $this->modelClass::find($this->recordId);
            if ($wfModel) {
                $workflowTransitions = $wfModel->getAvailableTransitions();
                $workflowCurrentState = $wfModel->getCurrentStateLabel();
            }
        }
        $this->addVariable('workflowTransitions', $workflowTransitions);
        $this->addVariable('workflowCurrentState', $workflowCurrentState);

        // Initialize View Config for JS fallback if needed
        $themeManager = new \Alxarafe\Infrastructure\Persistence\Frontend\ThemeManager();
        ...
        $this->viewConfig = base64_encode(json_encode([
            ...
            'translations' => \Alxarafe\Infrastructure\Lib\Trans::getAll(),
            'templates' => $templates
        ]));
    }
        ...
        foreach ($this->structConfig['edit']['head_buttons'] ?? [] as $btn) {
            ...
            if (($btn['name'] ?? '') === 'save') {
                if (empty($this->recordId) || $this->recordId === 'new') {
                    $label = \Alxarafe\Infrastructure\Lib\Trans::_('create');
                    $icon = 'fas fa-plus';
                    $type = 'success';
                } else {
                    $label = \Alxarafe\Infrastructure\Lib\Trans::_('save_changes');
                    $icon = 'fas fa-save';
                    $type = 'primary';
                }
            }
    ...
    {
        $genPath = __DIR__ . '/../../../Persistence/Frontend/TemplateGenerator.php';
        if (file_exists($genPath)) {
            require_once $genPath;
        } else {
            error_log("ResourceTrait: FORCED LOAD FAILED - File not found at $genPath");
        }
        $generator = new \Alxarafe\Infrastructure\Persistence\Frontend\TemplateGenerator();
        $descriptor = $this->getViewDescriptor();
    ...
    protected function setEditFields(array $fields, string $sectionId = 'main')
    {
        ...
        foreach ($fields as $key => $value) {
            ...
            if (is_object($value) && ($value instanceof AbstractField)) {
                ...
                if ($value instanceof \Alxarafe\Infrastructure\Component\Container\Panel) {
    ...
    protected function jsonResponse(mixed $data)
    {
        if (defined('ALX_TESTING')) {
            throw new \Alxarafe\Infrastructure\Testing\HttpResponseException($data);
        }

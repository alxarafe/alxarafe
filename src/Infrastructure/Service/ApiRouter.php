...
use Alxarafe\Infrastructure\Attribute\RequirePermission;
use Alxarafe\Infrastructure\Attribute\RequireRole;
...
class ApiRouter
{
    ...
    private function extractClassRoutes(string $filePath): array
    {
        ...
        try {
            ...
            foreach ($reflection->getAttributes(RequireRole::class, \ReflectionAttribute::IS_INSTANCEOF) as $attr) {
                $classRoles = array_merge($classRoles, $attr->newInstance()->roles);
            }
            foreach ($reflection->getAttributes(RequirePermission::class, \ReflectionAttribute::IS_INSTANCEOF) as $attr) {
                $classPermissions[] = $attr->newInstance()->permission;
            }

            // Method level routes
            foreach ($reflection->getMethods(ReflectionMethod::IS_PUBLIC) as $method) {
                $routeAttributes = $method->getAttributes(ApiRoute::class, \ReflectionAttribute::IS_INSTANCEOF);
                ...
                foreach ($routeAttributes as $attr) {
                    ...
                    foreach ($method->getAttributes(RequireRole::class, \ReflectionAttribute::IS_INSTANCEOF) as $roleAttr) {
                        $methodRoles = array_merge($methodRoles, $roleAttr->newInstance()->roles);
                    }
                    foreach ($method->getAttributes(RequirePermission::class, \ReflectionAttribute::IS_INSTANCEOF) as $permAttr) {

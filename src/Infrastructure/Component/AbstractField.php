...
abstract class AbstractField implements JsonSerializable
{
     ...
     * @param \Alxarafe\Infrastructure\Component\Enum\ActionPosition $position 'left' or 'right' (default: ActionPosition::Left)
     * @return self
     */
    public function addAction(string $icon, string $onclick, string $title = '', string $class = 'btn-outline-secondary', \Alxarafe\Infrastructure\Component\Enum\ActionPosition $position = \Alxarafe\Infrastructure\Component\Enum\ActionPosition::Left): self
    {
        $this->actions[] = [
            'icon' => $icon,
            'onclick' => $onclick,
    ...
    public function isVisible(): bool
    {
        ...
        try {
            if (class_exists('\Modules\Admin\Service\MenuManager')) {
                return \Modules\Admin\Service\MenuManager::isModuleEnabled($module);
            }
        } catch (\Throwable) {
    ...
    /**
     * @var \Alxarafe\Infrastructure\Persistence\Template|null Static renderer to avoid memory exhaustion
     */
    protected static ?\Alxarafe\Infrastructure\Persistence\Template $renderer = null;
    ...
    public function render(array $extraData = []): string
    {
        if (self::$renderer === null) {
            self::$renderer = new \Alxarafe\Infrastructure\Persistence\Template();

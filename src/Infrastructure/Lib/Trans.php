...
use Alxarafe\Infrastructure\Tools\Dispatcher\Dispatcher;
...
abstract class Trans
{
    ...
    public static function getAvailableLanguages(): array
    {
        // Try DB first
        try {
            if (class_exists(\Modules\Admin\Model\Language::class)) {
                $model = new \Modules\Admin\Model\Language();
                if ($model->existsInSchema()) {
                    return \Modules\Admin\Model\Language::getActiveList();
                }
            }
        } catch (\Throwable $e) {
    ...
    public static function getAvailableLanguagesWithFlags(): array
    {
        try {
            if (class_exists(\Modules\Admin\Model\Language::class)) {
                $model = new \Modules\Admin\Model\Language();
                if ($model->existsInSchema()) {
                    return \Modules\Admin\Model\Language::getActiveWithFlags();
                }
            }
        } catch (\Throwable $e) {
            // DB unavailable
    ...
    private static function loadLang(string $lang, string $folder): void
    {
        ...
        if (!file_exists($filename)) {
            Debug::message(self::_('trans_file_not_found') . ' ' . $filename);
            return;
        }

        $translates = Yaml::parseFile($filename);
        if (!is_array($translates)) {
            Debug::message(self::_('trans_invalid_yaml') . ' ' . $filename);
            return;
        }

...
class DependencyResolver
{
    ...
    private static function scan(): array
    {
        ...
        foreach ($routes as $route) {
            ...
            $nsPrefix = $route['namespace']; // 'CoreModules' or 'Modules'
    ...
    private static function extractUseStatements(string $filePath): array
    {
        ...
        // Match: use CoreModules\Admin\Model\Setting;
        if (preg_match_all('/^use\s+((?:Modules|CoreModules)\\\\[^;]+);/m', $content, $matches)) {
            $uses = $matches[1];
        }
    ...
    private static function resolveModuleFromNamespace(string $fqcn, array $moduleMap): ?string
    {
        ...
        $prefix = $parts[0]; // 'Modules' or 'CoreModules'
        $moduleName = $parts[1];

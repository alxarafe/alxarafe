...
abstract class ResourceController extends Controller
{
    ...
    protected function fetchListData(string $tabId): array
    {
        ...
        /** @var \Xnet\Core\xnet_model $model */
        ...
        if ($globalQuery && !empty($this->globalSearchFields)) {
            ...
            foreach ($this->globalSearchFields as $field) {
                // Use DBSchema if available for robust search, otherwise standard LIKE
                if (class_exists('\Xnet\Core\DBSchema') && method_exists('\Xnet\Core\DBSchema', 'search_diacritic_insensitive')) {
                    $searchParts[] = \Xnet\Core\DBSchema::search_diacritic_insensitive($field, $globalQuery);
                } else {
        ...
        foreach ($tabConfig['filters'] as $filter) {
            /** @var \Xnet\Core\Component\Filter\AbstractFilter $filter */
    ...
    protected function renderView()
    {
        ...
        $this->template = 'core/xnet_resource_view';

        $this->xnetConfig = base64_encode(json_encode([
            'mode' => $this->mode,
